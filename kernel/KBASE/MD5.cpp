...
std::string KMD5::GetFileMD5(const std::string& filefullpath)
{
	...
	unsigned char pBuffer[nBufferSize]={0};	//buffer for data read from the file

	//checksum the file in blocks of 1024 bytes
	while(!feof(f))
	{
		nLength=fread((unsigned char*)pBuffer,sizeof(unsigned char),nBufferSize,f);
		MD5.Update(pBuffer,nLength);
	}
	fclose(f);
